...
app.layout = html.Div([
    html.H1("Real-Time System Monitoring", style={'textAlign': 'center', 'color': '#003366'}),
...
def update_cpu_graph(n):
    ...
    fig = px.line(df, x='Time', y='CPU Usage (%)', title='CPU Usage Over Time',
                  markers=True, line_shape='linear', color_discrete_sequence=['#FF5733'])
    fig.update_layout(xaxis_title='Time (s)', yaxis_title='CPU Usage (%)', transition_duration=500,
                      plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(255,255,255,0.8)')
    return fig
...
def update_memory_graph(n):
    ...
    fig = px.line(df, x='Time', y='Memory Usage (%)', title='Memory Usage Over Time',
                  markers=True, line_shape='linear', color_discrete_sequence=['#33CFFF'])
...
def update_disk_graph(n):
    ...
    fig = px.line(df, x='Time', y='Disk Usage (%)', title='Disk Usage Over Time',
                  markers=True, line_shape='linear', color_discrete_sequence=['#6B8E23'])
...
def update_gpu_graph(n):
    ...
    if gpu_info:
        ...
        fig.update_layout(plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(255,255,255,0.8)')
    return fig
